...
std::wstring GetLastErrSzW()
{
    ...
    std::wstring Err = (BTCCWSTR)lpMsgBuf;
    LocalFree(lpMsgBuf);
    return Err;
}
...
BOOL Win32RandomAccessFile::_Init( BTCCWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
...
BOOL Win32MapFile::_Init( BTCCWSTR Path )
{
...
BOOL Win32FileLock::_Init( BTCCWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
